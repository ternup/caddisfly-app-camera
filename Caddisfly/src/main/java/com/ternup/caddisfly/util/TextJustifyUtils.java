...
public class TextJustifyUtils {
    ...
    final static float COMPLEXITY = 5.12f;
            //Reducing this will increase efficiency but will decrease effectiveness
    ...
    public static void justify(TextView textView) {
        ...
        for (int i = 0; i < blocks.length; i++) {
            ...
            wrappedObj = TextJustifyUtils
                    .createWrappedLine(block, paint, spaceOffset, textWrapWidth);
    ...
    public static Object[] createWrappedLine(String block, Paint paint, float spaceOffset,
            float maxWidth) {
        ...
        for (int x = 0; x < splits.length; x++) {
            if (p.measureText(splits[x]) > width) {
                ...
                for (int z = 0; z < microSplits.length; z++) {
                    smb_internal
                            .append(microSplits[z] + ((z + 1 < microSplits.length) ? SYSTEM_NEWLINE
                                    : ""));
                }
